    ...
    private fun initView() {
        NewGetXView(object : GetXListener {
            ...
            override fun onDataChange(view: NewGetXView) {
                ...
                data.modeDefault = (GetArchName.ModeDefault == modelType)
//                data.modeEasy = (GetArchName.ModeEasy == modelType)
...
//                data.function.addBinding = view.bindingBox.isSelected
                ...
                val list = ArrayList<TemplateInfo>().apply {
                    add(data.templatePage.apply { selected = (GetArchName.templatePage == templateType) })
                    add(data.templateComponent.apply { selected = (GetArchName.templateComponent == templateType) })
                    add(data.templateCustom.apply { selected = (GetArchName.templateCustom == templateType) })
                }
                for (item in list) {
...
//        }
    ...
    private fun getSuitableContent(inputFileName: String): String {
        ...
        //add lifecycle
        if (data.function.addLifecycle && inputFileName.contains("controller.dart")) {

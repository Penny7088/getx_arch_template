...
open class NewGetXView(private val getXListener: GetXListener) {
     ...
     */
    private var jDialog: JDialog = JDialog(JFrame(), "GetX Arch code gen")
...
//    lateinit var bindingBox: JCheckBox
    ...
    private fun setMode(container: Container) {
        ...
        //default model
        val defaultBtn = JRadioButton(GetArchName.ModeDefault, data.modeDefault)
        defaultBtn.actionCommand = GetArchName.ModeDefault
        ...
        //easy model
//        val easyBtn = JRadioButton(GetArchName.ModeEasy, data.modeEasy)
//        easyBtn.actionCommand = GetArchName.ModeEasy
...
//        modeGroup.add(easyBtn)
    ...
    private fun getMainFunction(): JPanel {
        ...
        //use folder
        folderBox = JCheckBox(GetArchName.mainUseFolder, data.function.useFolder)
        folderBox.addActionListener(actionChangeListener)
        setMargin(folderBox)
        main.add(folderBox)

        //use prefix
//        prefixBox = JCheckBox(GetArchName.mainUsePrefix, data.function.usePrefix)
...
//        main.add(bindingBox)
    ...
    private fun getMinorFunction(): JPanel {
        ...
        //add lifecycle
        lifecycleBox = JCheckBox(GetArchName.minorAddLifecycle, data.function.addLifecycle)
        lifecycleBox.addActionListener(actionChangeListener)
        setMargin(lifecycleBox)
        minor.add(lifecycleBox)

        //auto dispose
        disposeBox = JCheckBox(GetArchName.minorAutoDispose, data.function.autoDispose)
        disposeBox.addActionListener(actionChangeListener)
        setMargin(disposeBox)
        minor.add(disposeBox)

        //support lint normal
        lintNormBox = JCheckBox(GetArchName.minorLintNorm, data.function.lintNorm)
    ...
    private fun getTemplateFunction(): JPanel {
        ...
        //add page
        val pageBtn = JRadioButton(GetArchName.templatePage, data.templatePage.selected)
        pageBtn.actionCommand = GetArchName.templatePage
        pageBtn.addActionListener(actionChangeListener)
        setPadding(pageBtn)
        template.add(pageBtn)

        //add component
        val componentBtn = JRadioButton(GetArchName.templateComponent, data.templateComponent.selected)
        componentBtn.actionCommand = GetArchName.templateComponent
        componentBtn.addActionListener(actionChangeListener)
        setPadding(componentBtn)
        template.add(componentBtn)

        //add custom
        val customBtn = JRadioButton(GetArchName.templateCustom, data.templateCustom.selected)
        customBtn.actionCommand = GetArchName.templateCustom
